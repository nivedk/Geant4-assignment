...
G4VPhysicalVolume* B4cDetectorConstruction::DefineVolumes()
{
  // Geometry parameters
  fNofLayers = 1;
  G4double absoThickness = 0.000004*cm;
  G4double gapThickness =  0.1*nm;
  G4double calorSizeXY  = 3.*cm;
  ...
  auto absorberMaterial = G4Material::GetMaterial("G4_Au");
  auto gapMaterial = G4Material::GetMaterial("Galactic");
  auto detectorMaterial = G4Material::GetMaterial("G4_Pb");
  ...
  auto worldS 
    = new G4Box("World",           // its name
                 1000*worldSizeXY/2, 1000*worldSizeXY/2, 1000*worldSizeZ/2); // its size
  ...
  //  
  auto calorimeterS
    = new G4Tubs("Calorimeter", // name
    0.1*m, // inner radius
    0.12*m, // outer radius
    0.06*m, // Z half length
    0.14, // starting Phi
    6); // segment angle
  ...
  /*
  auto detectorCyl = new G4Tubs("cylDet", // name
    0.1*m, // inner radius
    0.12*m, // outer radius
    0.06*m, // Z half length
    0.14, // starting Phi
  ...
  auto cylindricalDetector
    = new G4LogicalVolume(
                 detectorCyl,     // its solid
  ...
  new G4PVPlacement(
                 ...
                 G4ThreeVector(1.*m, 1.*m, 1.*m),  // at (0,0,0)
                 cylindricalDetector,          // its logical volume                         
                 "cylDet",    // its name
                 worldLV,          // its mother  volume
                 false,            // no boolean operation
                 0,                // copy number
  ...
  */
  ...
  //cylindricalDetector->SetVisAttributes(simpleBoxVisAtt);
